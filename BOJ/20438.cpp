...
int main() {
	...
	while(q--){
		...
		add=x; //출석코드를 받은 학생 입장 번호를 add에 넣어놓고 누적으로 더해감 
		
		while(x<=n+2){
			if(sleep[x]){ //중간에 졸고있는 사람이 있으면 출석체크X
				x+=add;  
				continue;
			}
			ch[x]=1, //졸고있는 학생 아니면 출석코드를 받을 학생 정보를 ch배열에 저장 
	...
	for(int i=3; i<=n+2; i++){ //출석되지 않은 학생 수 계산 
		...
		sum[i]=sum[i-1]+attendX; //벡터에 누적합 계산. 
		//printf("%d: %d\n", i, sum[i]); //확인출력 
	}
